...
def run_10_advanced_tests():
    ...
    r_subtokens = SphereGeometry.normalize(
        S + np.random.randn(100, dim) * subtoken_noise_std
    )
    r_sentences = SphereGeometry.normalize(
        S + np.random.randn(100, dim) * sentence_noise_std
    )
    ...
    print(
        f"  -> Дисперсия расстояний для целых предложений:        {var_sentences:.6f}"
    )
    print(
        f"  -> Снижение шума при переходе к предложениям:        {var_subtokens/var_sentences:.1f}x раз"
    )
    ...
    U_tangent = np.array(
        [SphereGeometry.log_map(S, SphereGeometry.normalize(a)) for a in attacks_matrix]
    )
    ...
    print(
        f"  -> Макс. невязка ортогональности v_perp к подпространству U_thr: {ortho_check:.2e}"
    )
    ...
    for a in angles:
        ...
        print(
            f"  -> Скалярное произведение <S, R>: {-a:.8f} | Вычисления корректны: {not has_nan}"
        )
    ...
    dataset = SphereGeometry.normalize(
        np.random.randn(300, dim) * 0.1 + np.ones(dim) * 0.3
    )
    ...
    useful_recovery_cos = np.dot(v_p, SphereGeometry.log_map(S, V_useful)) / (
        np.linalg.norm(v_p) * SphereGeometry.geodesic_dist(S, V_useful)
    )
    print(
        f"  -> Точность восстановления полезного сигнала (Cos Similarity): {useful_recovery_cos:.6f}"
    )
    ...
    print(
        f"  -> Прирост качества детекции:              +{(auc_rsfi - auc_cos)*100:.2f}%"
    )
    print(
        "  [РЕЗУЛЬТАТ]: Продемонстрировано превосходство RSFI над стандартными метриками."
    )
