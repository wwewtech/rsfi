...
1. **По схеме доступа к модели (Access Level):**
   ...
   - *Black-Box (Методы «Черного ящика» / API-level):* Работают исключительно на уровне входных текстовых последовательностей и выходных токенов/эмбеддингов через пользовательский или программный интерфейс (API), не требуя доступа к архитектуре и внутренним весам нейросети [6, 13].
