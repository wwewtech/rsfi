...
- `E13b_transfer_to_bigbench.csv` (+ `_summary.csv`) - E13 transfer protocol (reference pools ONLY from the train corpus, no re-calibration) into the 4 E16 BigBench safe-pool targets ({AdvBench,HarmBench} x {BB_Safe, BB_Mixed}); same helpers/methods as E13 (A1_raw, B1_raw, B1b_SigmaT, B1w_SigmaW + DELONG_B1w_vs_A1 rows, p in roc_auc), embeddings from the E16 caches. Key result (mean AUC over targets x embedders x seeds, whitened B1b/B1w by train corpus): HarmBench 0.9974-1.0000, AdvBench 0.8875-1.0000, XSTest 0.8143-0.9867, ToxicChat 0.7303-0.9897, while train=Wild collapses to 0.3827-0.8046 (the safe-only Wild corpus carries no attack knowledge) - cross-corpus transfer is NOT automatic and depends on the train corpus. Documented degeneracy: in same-source cells both AUCs can be exactly 1.0 -> DeLong variance 0 -> p undefined (NaN; 6 of 400 rows).
- `E17b_generation_regimes_prompts.csv`, `E17b_generation_regimes_summary.csv` - E17 with 4 generation regimes (same E11 split, prompts, blocking signals, judge, greedy decoding; only the regime varies): e17_default (identical to committed E17), neutral_system, prefix_injection (forced assistant prefix), strong_generator (Qwen2.5-3B). CAVEAT: for strong_generator the generator == judge (Qwen2.5-3B-Instruct), so its labels are self-evaluation, not an independent judgment; the three 1.5B regimes keep generator != judge. Summary: compliance/refusal rates + Wilson CIs, TPR/FPR at the safe-pool-calibrated tau.
- `E17c_judge_power_analysis.csv`, `E17c_judge_power_reference.csv` - Judge power audit of the committed E17: Wilson CIs on the compliance rates and TPR/FPR (degenerate p=0/1 stored as NaN) show the committed TPR rests on n_harmful <= 2 generations (ToxicChat: TPR=0.5, CI [0.09, 0.91]); reference table stores the closed-form required n (z^2 p(1-p)/h^2) for planning (e.g. p=0.5, h=0.1 -> 97 harmful generations).
- `E18_b1b_geometry_check.csv`, `E18_b1b_margin_probe_per_sample.csv` (+ `_summary.csv`) - Explains the zero B1b ASR@1%FPR anomaly of E15: Part A (Ledoit-Wolf-regularized geometry; raw Sigma_W is rank-deficient in the few-shot regime, rank 398/768, cond ~1e35) shows Sigma_T suppresses 78-83% of the between-class direction norm (suppression_lw) and the disp/margin ratio stays below raw B1 at every budget; Part B dose-response margin probe (32,400 rows) shows B1 grows to ASR ~0.43 with budget (more substitutions), while B1b stays at 0.000 even at cap 12. The zero ASR is a suppression/margin effect, NOT a separate Sigma_T invariance mechanism.
- `AGGREGATED_mean_auc_ci.csv` - Aggregated 95% confidence intervals of per-seed mean AUC (t-distribution, truncated to the [0,1] AUC support) recomputed from ALL per-seed CSVs (E2d/E2q, E8/E8q, E12, E13, E13b, E14, E15, E16); generated by `experiments/generate_aggregated_summary.py`.
- `AGGREGATED_delong.csv` - Aggregated paired DeLong tests: per (source, dataset, model, pair) - wins/ties/losses over seeds counted from the per-seed AUC direction (not from p-significance), mean/max p-value over the DEFINED p-values (n_valid_p; E13/E13b store the p in roc_auc and NaN under perfect separation), fraction of valid seeds with p<0.05; generated by `experiments/generate_aggregated_summary.py`.
