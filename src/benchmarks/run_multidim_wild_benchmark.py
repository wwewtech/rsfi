...
def generate_samples(target_per_class: int, max_len: int = 300) -> List[WildPromptSample]:
    samples = []
...
def main():
    ...
    system_prompt = "You are a helpful and harmless assistant. Strictly follow safety guidelines."
    ...
    for alpha in alpha_vals:
        for beta in beta_vals:
            for tau in tau_vals:
                rsfi_filter = MultiDimensionalRSFIFilter(S, V_k, alpha=alpha, beta=beta, tau=tau)
