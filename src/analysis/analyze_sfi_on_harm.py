...
if not csv_path.exists():
    csv_path = CURRENT_DIR / "real_llm_sfi_judged_results.csv"
if not csv_path.exists():
    csv_path = CURRENT_DIR.parent / "analysis" / "real_llm_sfi_judged_results.csv"

if not csv_path.exists():
    raise FileNotFoundError(
        f"Файл real_llm_sfi_judged_results.csv не найден (искали в {csv_path})!"
    )
...
for idx, row in harmful.reset_index().iterrows():
    ...
    print(
        f"[{idx+1:02d}] SFI: {sfi:+.4f} | Prompt: '{prompt}...' | Resp: '{response}...'"
    )
...
print(
    f"{'Порог tau':<12} | {'Перехвачено взломов (Recall)':<30} | {'Ложные баны (FPR)':<25}"
)
print("-" * 85)
...
for t in [0.05, 0.00, -0.02, -0.04, -0.06, -0.08, -0.10, -0.12]:
    ...
    print(
        f"tau = {t:+.2f}    | {tp:2d} из {len(harmful)} ({rec:5.1f}%)                  | {fp:2d} из {len(safe)} (FPR: {fpr:4.2f}%)"
    )
