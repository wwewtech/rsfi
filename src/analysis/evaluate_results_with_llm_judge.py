...
if not CSV_INPUT_PATH.exists():
    CSV_INPUT_PATH = (
        CURRENT_DIR.parent / "benchmarks" / "real_llm_sfi_calibrated_results.csv"
    )
if not CSV_INPUT_PATH.exists():
    CSV_INPUT_PATH = Path("real_llm_sfi_calibrated_results.csv")
...
if __name__ == "__main__":
    ...
    if not CSV_INPUT_PATH.exists():
        raise FileNotFoundError(
            f"Файл {CSV_INPUT_PATH} не найден. Сначала запустите бенчмарк."
        )
    ...
    for idx, row in df.iterrows():
        ...
        if (idx + 1) % 100 == 0 or idx == len(df) - 1:
            ...
            print(
                f"Оценено: {idx+1}/{len(df)} | Скорость: {sec_per:.2f} сек/ответ | Текущая метка: {label}"
            )
    ...
    print(
        f"  -> Безопасные отказы Qwen2.5 (REFUSAL)            : {refusals_count} ({refusals_count/total*100:.1f}%)"
    )
    print(
        f"  -> Безопасные обычные ответы (BENIGN_RESPONSE)    : {benign_count} ({benign_count/total*100:.1f}%)"
    )
    print(
        f"  -> РЕАЛЬНЫЕ ВЗЛОМЫ / ВРЕД (HARMFUL_COMPLIANCE)    : {harmful_count} ({harmful_count/total*100:.1f}%)"
    )
    print("-" * 85)
    ...
    print(
        f"True Negatives (Безопасные ответы пропущены)    : {tn} из {refusals_count + benign_count}"
    )
    print(
        f"False Positives (Ложные баны безопасных ответов): {fp} (FPR: {fpr*100:.2f}%)"
    )
    print(f"False Negatives (Пропущенный реальный вред)     : {fn}")
    print("-" * 85)
    print(
        f"Истинная точность системы (Accuracy)             : {accuracy:.4f} ({accuracy*100:.2f}%)"
    )
    print(f"Истинная точность детекции (Precision)           : {precision:.4f}")
    print(
        f"Истинная полнота нейтрализации (Recall)          : {recall:.4f} ({recall*100:.2f}%)"
    )
