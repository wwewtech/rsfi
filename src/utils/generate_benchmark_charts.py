...
def generate_charts(out_dir: str = "docs/figures"):
    ...
    for name, auc_val, color, ls in models_roc:
        ...
        ax1.plot(fpr, tpr, label=f"{name} (AUC = {auc_val:.4f})", color=color, linestyle=ls, linewidth=2.2)
    ...
    for name, auc_val, color, ls in models_roc:
        ...
        pr_auc = auc(rec, prec)
        ax2.plot(rec, prec, label=f"{name} (PR-AUC = {pr_auc:.4f})", color=color, linestyle=ls, linewidth=2.2)
    ...
    sns.kdeplot(safe_scores, fill=True, color="#1f77b4", alpha=0.4, label="Safe / Benign Prompts (N=2,500)", ax=ax, lw=2)
    sns.kdeplot(mal_scores, fill=True, color="#d62728", alpha=0.4, label="Malicious Jailbreaks (N=2,500)", ax=ax, lw=2)

    ax.axvline(x=0.0, color="black", linestyle="--", linewidth=2.0, label=r"Decision Threshold $\tau^* = 0.00$")

    ax.set_xlabel("RSFI Fidelity Index $\\text{RSFI}(r)$", fontweight="bold")
    ax.set_ylabel("Kernel Density Estimate (KDE)", fontweight="bold")
    ax.set_title("RSFI Score Separation: Safe vs. Malicious In-The-Wild Prompts", fontweight="bold", pad=12)
    ...
    n, bins, patches = ax.hist(latencies_us, bins=40, color="#2ca02c", edgecolor="#1b661b", alpha=0.75, density=True)

    mean_lat = np.mean(latencies_us)
    p95_lat = np.percentile(latencies_us, 95)

    ax.axvline(mean_lat, color="#d62728", linestyle="-", linewidth=2, label=f"Mean Latency = {mean_lat:.1f} $\\mu s$")
    ax.axvline(p95_lat, color="#ff7f0e", linestyle="--", linewidth=2, label=f"P95 Latency = {p95_lat:.1f} $\\mu s$")

    ax.set_xlabel("Evaluation Latency per Prompt (Microseconds $\\mu s$)", fontweight="bold")
    ax.set_ylabel("Probability Density", fontweight="bold")
    ax.set_title("Microsecond Latency Profile of RSFI Tangent Subspace Filter", fontweight="bold", pad=12)
    ...
    sns.heatmap(grid_auc, annot=True, fmt=".4f", cmap="YlGnBu", xticklabels=taus, yticklabels=alphas, ax=ax, cbar_kws={'label': 'ROC-AUC Score'})

    ax.set_xlabel(r"Decision Threshold $\tau$", fontweight="bold")
    ax.set_ylabel(r"Threat Penalty Weight $\alpha$", fontweight="bold")
    ax.set_title("Grid Search Optimization: Threat Penalty $\\alpha$ vs. Threshold $\\tau$", fontweight="bold", pad=12)
    ...
    if sweep_csv.exists():
        ...
        ax.plot(ks, aucs_50, marker='s', linewidth=2.2, markersize=7, label='RSFI Subspace (N_ref = 50 prompts)')
        ax.plot(ks, aucs_100, marker='^', linewidth=2.2, markersize=7, label='RSFI Subspace (N_ref = 100 prompts)')
        ax.plot(ks, aucs_200, marker='o', linewidth=2.5, markersize=8, label='RSFI Subspace (N_ref = 200 prompts)')
    ...
    ax.set_title('Monotonic ROC-AUC Growth vs Threat Subspace Dimension (k)', fontweight='bold', pad=12)
