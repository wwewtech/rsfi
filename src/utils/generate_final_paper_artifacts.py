...
def plot_perfect_fig1(df_judged):
    ...
    safe_scores = df_judged[df_judged["judge_label"] != "HARMFUL_COMPLIANCE"]["sfi_score"].values
    harm_scores = df_judged[df_judged["judge_label"] == "HARMFUL_COMPLIANCE"]["sfi_score"].values
    
    # Строим гистограммы с нормировкой плотности (density=True)
    ax.hist(safe_scores, bins=30, density=True, alpha=0.5, color='#1f77b4', edgecolor='#1f77b4', label='Safe / Refusal Responses (N=990)')
    ax.hist(harm_scores, bins=10, density=True, alpha=0.7, color='#d62728', edgecolor='#d62728', label='Actual Harmful Exploits (N=10)')
    
    # Линия калиброванного порога
    ax.axvline(x=-0.10, color='black', linestyle='--', linewidth=2, label=r'Optimal Threshold $\tau^* = -0.10$')
    
    ax.set_xlabel('System Fidelity Index (SFI)', fontweight='bold')
    ax.set_ylabel('Normalized Probability Density', fontweight='bold')
    ax.set_title('Figure 1: SFI Density Separation on Real Qwen2.5 Generations', fontsize=12, pad=12)
...
def plot_perfect_fig2(df_judged):
    ...
    ax.plot(fpr, tpr, color='#2ca02c', lw=2.5, label=f'SFI Guardrail (AUC = {roc_auc:.4f})')
    ax.plot([0, 1], [0, 1], color='#7f7f7f', lw=1.5, linestyle='--', label='Random Classifier')
    
    ax.set_xlim([-0.02, 1.02])
    ax.set_ylim([-0.02, 1.05])
    ax.set_xlabel('False Positive Rate (FPR)', fontweight='bold')
    ax.set_ylabel('True Positive Rate (Recall)', fontweight='bold')
    ax.set_title('Figure 2: ROC Curve for SFI Exploit Interception', fontsize=12, pad=12)
...
def plot_perfect_fig3():
    ...
    bplot = ax.boxplot(data_to_plot, patch_artist=True, tick_labels=['SFI on GPU (CUDA)', 'SFI on CPU (Host)'],
    ...
    plt.tight_layout()
    plt.savefig(CURRENT_DIR / "fig3_latency_performance.png", dpi=300, bbox_inches='tight')
    plt.close()
...
def plot_perfect_fig4():
    ...
    categories = ['Standard Attacks', 'Copyright Violation', 'Contextual Exploits', 'Safe Control Group']
    mitigation_rates = [100.0, 100.0, 100.0, 0.0] # 0.0% ложных банов в группе Safe
    colors = ['#1f77b4', '#1f77b4', '#1f77b4', '#2ca02c']
    
    bars = ax.bar(categories, mitigation_rates, color=colors, edgecolor='black', width=0.55, alpha=0.85)
    ...
    ax.set_title('Figure 4: SFI Performance Across HarmBench Risk Categories', fontsize=12, pad=12)
    ax.grid(True, linestyle=':', alpha=0.5, axis='y')
    
    # Поворачиваем подписи оси X, чтобы не обрезались
    plt.xticks(rotation=10, ha='right')
    plt.tight_layout()
    plt.savefig(CURRENT_DIR / "fig4_harmbench_categories.png", dpi=300, bbox_inches='tight')
    plt.close()
